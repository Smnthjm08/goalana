...
| 🚧 | **P2-2** | **Headline "permissionless settlement" in README + UI.** `settle_market` takes **no authority signer** — anyone with the genuine TxLINE proof can settle any market (verified: `SettleMarket` accounts = market + oracle program + daily-roots PDA, nothing else). This is the track's literal words — _"a user or keeper bot triggers your contract"_ — and it's currently invisible: README frames settlement as "the backend does it". One README paragraph + a "anyone can settle this market — the keeper is a convenience, not an authority" line on the proof receipt. | 1h | Strongest remaining positioning win, zero code risk. Same for `claim_*`: permissionless, user-pulled. **UI line done 2026-07-18** (see progress log) — README paragraph still open. |
| 🚧 | **P2-3** | **"Judging-window" note in README + a banner rule in the app**: TxLINE free-tier access ends at the deadline, so during review the health indicator may show red and no new fixtures/odds flow; all proofs, evidence txs, and settled markets are **persisted** and fully inspectable. | 0.5h | Prevents a judge from reading a dead feed as a broken app — the single cheapest misjudgment insurance available. **App banner done 2026-07-18** (see progress log) — README note still open. |
...
| 🚧 | **A2** | **Permissionless settlement is real but unmarketed** (see P2-2). Also true of `claim_winnings`/`claim_refund` (user-signed pull, no house involvement) — the _only_ house-gated instructions are create/lock/cancel, and that's exactly the "house makes markets, cryptography settles them" story. | README/UI positioning only — this is P2-2. UI half done 2026-07-18, README half still open. |
...
| 🚧 | **B3** | **Position rent is never reclaimable** — no close instruction exists (that's what `close_bet` was presumably scaffolded for). After claim, ~0.002 SOL stays locked per position forever. | Add `close_position` (`constraint = position.claimed`, `close = user`). Nice-to-have; skip without guilt. **Instruction written + `anchor build`/`anchor test` (26/26) verified 2026-07-18 — see progress log. Not deployed**, per this section's own gate: still not judge-visible until redeployed, which stays held until the France v England settlement evidence lands. |
...
| 🚧 | **B5** | Consistency nit: `lock_market`/`cancel_market` take `market` without re-deriving PDA seeds (bet/claim/settle all do). Safe today — `Account<Market>` enforces owner+discriminator and both are authority-gated — but inconsistent. | Add the `seeds`/`bump` constraints when B1/B2 happen anyway. Not worth a deploy alone. **Constraints added + `anchor build`/`anchor test` (26/26) verified 2026-07-18 — see progress log. Not deployed** — same posture as B3, done standalone rather than bundled with B1/B2 since it's pure validation-tightening with no authorization change. |
...
- **2026-07-18 — P2-2 and P2-3 code portions done** (user asked for 3 code tasks, not docs; A2/P2-2/P2-3 stay 🚧 until their README halves land). Full writeup in v2-todo.md's 2026-07-18 (item 20, plus v3 P2-2/P2-3) progress log:
  - **P2-2 UI**: `settlement-proof-receipt.tsx` now states the permissionless-settlement fact directly on the receipt, shown only for actual settlements (`mode="settled"`), not proof previews.
  - **P2-3 app banner**: new `judging-window-banner.tsx`, mounted in `app/layout.tsx`, polls `/api/health` and shows a slim amber notice only when the TxLINE stream is disconnected or the backend is unreachable — explains the free-tier judging-window caveat instead of leaving a judge to read a red indicator as a broken app.
  - Both README halves (the P2-2 paragraph, the P2-3 note) remain open — out of scope for this pass by explicit instruction.
- **2026-07-18 — B3 written and build/test-verified, deliberately NOT deployed.** Second round of "3 code tasks"; B3 was flagged before starting (jumps this section's "post-settlement, P0–P2 done" gate) and confirmed by explicit instruction, same pattern as the Protocol Inspector entry above. New `close_position` instruction: `instructions/close_position.rs` (`ClosePosition` accounts struct — `constraint = position.claimed`, `constraint = position.user == user.key()`, `close = user`; derives its seeds off `position.market` so it needs no separate `Market` account), new `PositionNotClaimed` error, wired into `instructions.rs`/`lib.rs` like every other instruction. Doesn't touch `lock_market`/`cancel_market`/any currently-live instruction, and doesn't touch any of §0's explicitly frozen files. Verified with the full local suite, not just a build: `anchor build` clean, `anchor test` **26/26 passing** (no regressions), and confirmed via the fresh `target/idl/goalana_program.json` that `close_position`'s IDL entry has exactly the expected PDA seed structure. **Left undeployed** — writing/testing code locally is safe regardless of the redeploy gate, but shipping it to Devnet is exactly what the gate defers; nothing here changes that calculus.
  - Same pass also shipped two non-Anchor items from v2-todo.md (lifecycle-timeline stage animations, SSE reconnect exponential backoff cap in `odds.worker.ts`/`scorer.worker.ts`) — full writeup in v2-todo.md's 2026-07-18 (lifecycle animations, SSE backoff cap, B3 close_position) progress log.
- **2026-07-18 — Third round of "3 code tasks."** User asked again; the safe/additive pool was thin enough this time that only 2 were picked (flagged and confirmed before starting rather than stretching for a weak third — B1/B2 were the only other real candidates and both change already-live instruction authorization logic, which felt like the wrong thing to reach for on match day):
  - **B5 shipped, not deployed.** Added `seeds = [b"market", market.fixture_id.to_le_bytes().as_ref(), market.predicate_hash.as_ref()], bump = market.bump` to both `LockMarket` and `CancelMarket`'s `market` account — matches the pattern every other instruction (`place_bet`/`claim_winnings`/`claim_refund`/`settle_market`) already uses. Pure validation-tightening: `create_market` only ever creates a `Market` via `init` with these exact seeds, so this doesn't change behavior for any legitimate caller, only makes an implicit invariant explicit. `anchor build` clean, `anchor test` **26/26 passing** (including the `Open -> Locked -> success` / `Locked -> Locked -> fail` / `Cancelled -> Locked -> fail` cases that exercise these exact accounts). Not deployed, same reasoning as B3.
  - **RPC retry for `fetchMarketAccount`**, scoped narrowly. `apps/api/src/services/goalana.service.ts`: new local `withReadRetry()` helper (3 attempts, exponential backoff from 500ms), applied only to `fetchMarketAccount`'s `program.account.market.fetch()` call — the one plain read in this file. Deliberately **not** applied to `lockMarketOnChain`/`cancelMarketOnChain`/`settleMarketOnChain`/`placeBetOnChain`/`claimWinningsOnChain` in the same file: blindly retrying a `.rpc()` write risks a double-submit if the first attempt actually landed but the client-side confirmation call itself failed, whereas retrying a read is unconditionally safe. `bun run --filter=api typecheck` clean; `fetchMarketAccount`'s return type is unchanged so no caller needed updating. (API's lint script is broken in this environment — `eslint: command not found`, confirmed pre-existing via `git stash` in an earlier entry above — not something this change touches.)
  - Neither change touches any of §0's explicitly frozen files.
- **2026-07-18 — Fixed SDK IDL synchronization bug.** `sync-idl.ts` previously rewrote every `address` field when copying Anchor's build artifacts into `packages/goalana-sdk`, corrupting fixed-address accounts (`system_program`, `txoracle_program`) with the goalana program's own address. The sync now updates only the program address, preserving Anchor-generated fixed-address metadata. This also surfaced an initialization-path bug (`createMarketForFixture` using the stricter `.accounts()` instead of `.accountsPartial()`) relying on implicit account resolution, which was corrected. Full writeup in v2-todo.md's 2026-07-18 (SDK IDL sync bug) progress log.
