...
| ✅ | **1c** | **Execute France v England cancel → recreate** — was already run live during the production validation pass (`--execute --rebet 0.05`); this checkbox was just stale. Confirmed live again just now: all 5 old markets `CANCELLED`, all 5 recreated with keys 1/2 `OPEN`, plus the 2 parametric prop markets from item 18 — 12 markets total on fixture `18257865`, none still on the old corners predicate. | 🔥🔥🔥 | 0.25d | Med | Makes the genuine on-chain settlement (2026-07-18) resolve on goals, not corners. **Real remaining blocker (not code): the external bettor's 1.051 SOL sits on the now-cancelled old HOME_WIN market — only that wallet can call `claim_refund`; Goalana's backend cannot act on their behalf.** The app already surfaces a Claim Refund button on the cancelled card. |
...
| ⚠️ | **11** | ~~Parametric prop markets (corners / cards)~~ — **BLOCKED as specified; intent delivered via #12's evidence panel** | 🔥🔥🔥 | — | — | TxLINE prices **no corners/cards odds** (only 1X2 / OVERUNDER / ASIANHANDICAP, all goals-based), and market creation is odds-gated — so tradeable prop markets can't exist. The _claim_ ("engine settles any stat") is now proven on-chain instead: real Devnet `validate_stat` txs accepting genuine **corners** and **cards** proofs. See progress log. |
...
3. **Item 14 (Protocol Inspector)** _or_ **Item 13 (Agent API)** — pick one as the "distinctive second wedge" based on whether the demo audience skews verifier (Inspector) or ecosystem/agents (API).
...
- ~~**#8 RISKS.md** + tampered-proof-rejection writeup + `settle_market` compute-unit stat~~ ✅ done.
...
|:---:|---|---|---|
| ✅ | **Protocol Inspector** (`/inspector`) — live config PDA, every Market (status/predicate/pools/`settle_after`), vault balances, `daily_scores_roots` freshness | Own addition (§5 ④), item 14 | "Inspect the whole protocol yourself." Pure on-chain reads, low risk. Pairs with the receipt: receipt proves _one_ settlement, inspector proves the _system_. |
| ⏳ | **Tampered proof demo** — submit `settle_market` with a mutated proof on Devnet, show the `validate_stat` CPI revert + logs + failed-tx link | Final Whistle (§1 A), item 12 | Strongest trust artifact in the field. Already proven 26/26 on localnet — this surfaces it as a _real failed Devnet tx_. Hits the track's custom-check-gate bonus. |
...
| ⏳ | **AI Agent API** — discover markets, read positions, build _unsigned_ bet/claim txs, `llm.txt`/OpenAPI descriptor | Track brief (§5 ②), item 13 | Track explicitly names AI agents as eligible builders; almost nobody will do it. Unsigned-tx builders mean we never custody agent keys. |
...
| ✅ | **Lifecycle animations** | `market-lifecycle-timeline.tsx` | Animate Create → Bet → Lock → Settle → Claim transitions as they land. Done 2026-07-18 — see progress log. |
...
- **No retry on individual RPC/DB calls** — self-heals within ~60s via the idempotent lifecycle cron, but a persistently-failing RPC endpoint gets hit every minute with no backoff. **Partially addressed 2026-07-18**: `fetchMarketAccount` (the read every lock/settle decision starts from) now retries with exponential backoff — see v3-todo.md's 2026-07-18 "Third round" progress log. Write calls (`lockMarketOnChain`/`cancelMarketOnChain`/`settleMarketOnChain`/etc.) deliberately still don't retry — a blind retry on a `.rpc()` write risks a double-submit if the first attempt actually landed on-chain but the confirmation call itself failed.
...
| :---: | --- | ------ | :---: | :---: | :---: | ------ |
| ✅ | **18** | **Parametric prop markets v2 — unpriced pari-mutuel** + **automated creation for every future fixture.** Created 2 real markets on France v England (18257865): "Total corners > 9.5" (keys 7+8, add > 9) and "Total cards > 3.5" (keys 3+4, add > 3), **without** TxLINE reference odds — labelled "Unpriced — the pool sets the price". Then wired the same predicate pair into the market-discovery cron itself, so every future fixture gets these two prop markets automatically, with no manual script run needed. | 🔥🔥🔥 | 0.5–1d | Med | Turns "the engine _could_ settle any stat" into **live prop markets that genuinely settle** — the track's literal suggested idea, end-to-end, and now a permanent feature rather than a one-off. Safety: the 5 goals markets are untouched; if a prop market misbehaves, `cancel_market` + refund is a proven path. |
...
| ✅ | **20** | **Pool-implied probability vs TxLINE reference** on market cards: "Pool implies 62% YES · TxLINE 58%" (pool pct = totalYes/(totalYes+totalNo), already on-chain; reference pct already stored). Delta badge when they diverge. | 🔥 | 2h | Low | Direct hit on "Prediction Market Viewer… updating implied probabilities using the real-time feed" — and it makes the pari-mutuel mechanism legible on camera. Done 2026-07-18 — see progress log. |
...
### Progress log — 2026-07-17 (item 18)

- **Time-math correction:** current UTC clock is 2026-07-17 evening — "TODAY" in this section's header referred to IST-local date (already past midnight IST), not the UTC date. Kickoff (2026-07-18T21:00Z) was never ~90 minutes away as first assessed mid-session; it's ~25h out from when item 18 shipped. No harm done — item 18 landed correctly either way — but noting it so the "hard deadline" framing above isn't taken as more urgent than it is.
- **Schema:** migration `nullable_initial_odds` — `Market.initialYesPct`/`initialNoPct` made nullable (was a hard `Float`), applied to the production Neon DB. Null now means "unpriced — pool sets the price," distinct from every priced market.
- **Manual creation, then automated.** First created the two markets by hand via a new one-off `apps/api/src/scripts/create-prop-markets.ts` (dry-run by default) against France v England (18257865):
  - `TOTAL_CORNERS_OVER_9_5` — PDA `3S7Qfd5goxGV2qWA6nJczd9UsRqPB93Cks5XTDWQtcPU`, [create_market tx](https://explorer.solana.com/tx/5FqQqH7fGiocMwdoLg4d7Q43aZm4xQW9fksXo5GJncfpNjm73R6Q6GKrEGHdvoEa6icX1KLuJX3WS32ViFLe1YZw?cluster=devnet)
  - `TOTAL_CARDS_OVER_3_5` — PDA `8nhWLYnwHuhwpoA3ptAzJr1VLb7dQe9iVeWvg74iy6x4`, [create_market tx](https://explorer.solana.com/tx/5PfBFxKNrk3JVmXtD1dcMkET3G8EsTH3Tc8oLtagv5A5BaBkenA9MwB8rXKKiGXx7P9GUA3DGwqbP8k9QkbspXbV?cluster=devnet)
- **Then made it permanent (user-requested follow-up).** The one-off script only ever covered this one hardcoded fixture — nothing would create prop markets for any future match. Moved the same predicate pair into `market-definitions.ts` as `PARAMETRIC_PROP_MARKETS`, and added `createParametricPropMarketsForFixture()` in `market.service.ts`, called unconditionally (independent of any TxLINE odds row — corners/cards have none) for every fixture the existing `market.cron.ts` → `processMarketsForUpcomingFixtures()` already discovers in its 24h lookahead. Every future fixture now gets both prop markets automatically, same as the five odds-driven goals markets — no manual script run needed going forward.
- **Idempotency verified live:** directly invoked the new function against France v England (which already has both markets) — 12 market rows before, 12 after, both logged "already exists, skipping," zero duplicate on-chain txs or DB rows. Safe to run on every cron tick indefinitely, matching the retry-safe pattern already established for the goals-market discovery loop.
- **Frontend:** `apps/web/app/fixtures/[fixtureId]/page.tsx` — new "PARAMETRIC PROPS" market-card group, "UNPRICED — THE POOL SETS THE PRICE" label in place of the TxLINE-reference caption, YES/NO % computed from the live on-chain pool split (`totalYes`/`totalNo`) instead of a null TxLINE percentage, and the odds-delta badge suppressed for unpriced markets (no opening reference to diff against). Typecheck 6/6 throughout; `/fixtures/18257865` verified 200 with both markets rendering via the live API.
- **The one-off script (`create-prop-markets.ts`) is now superseded** by the cron path for anything going forward, but left in place — harmless, and useful as a manual override/reference if the cron's 24h window ever needs to be bypassed for a specific fixture.

### Progress log — 2026-07-17 (items 1c, 8, 19)

- **#1c was already done — just a stale checkbox.** Re-verified live against `GET /api/fixtures/18257865`: all 5 old markets `CANCELLED`, all 5 recreated markets (keys 1/2) `OPEN`, plus the 2 parametric prop markets from item 18 — 12 markets total, none on the old corners predicate. The recreate itself ran during the "Production validation pass" (§ above) on 2026-07-17; this pass just corrected the roadmap table to reflect it. **The one real open item is not code**: the external wallet holding 1.051 SOL on the now-cancelled old `HOME_WIN` market must call `claim_refund` itself — Goalana's backend has no path to act on their behalf, by design.
- **#8 — `RISKS.md` written** at the repo root. Consolidates material that already existed scattered across this doc and `v3-todo.md` into one judge-facing document: (1) the tampered-proof-rejection evidence table (same 5 real Devnet signatures as the README's Proof Integrity section) with an explanation of *why* it had to run as a standalone `validate_stat` call rather than through a real `settle_market` failure; (2) the measured `validate_stat` compute-cost numbers across both finished fixtures (goals 131,986/198,959 CU, corners 200,460/198,965 CU, cards 200,458/198,963 CU) and why `SETTLE_COMPUTE_UNIT_LIMIT=400,000` was necessary; (3) the known localnet-mock-oracle limitation; (4) the house trust surface (`cancel_market`/`lock_market` ungated, but fund-safe — matches `v3-todo.md` §4 A3/B1/B2, not fixed pre-submission per the redeploy gate); (5) smaller issues — vault dust, unreclaimed position rent, and the still-unwired `close_bet.rs` dead stub (confirmed still not registered in `lib.rs`); (6) operational risks (SSE backoff, RPC retry); (7) the live external-bettor refund from #1c. Linked from README's Docs list.
- **#19 — "Built to the track sheet" section added to README**, right after the On-Chain Evidence section (so it can link straight into evidence anchors already on the page). Condenses §6's two tables: architectural considerations (all ✅) and the ideas-to-get-started coverage map, each row naming the shipped feature and linking to real evidence. Marked the Parametric Sports Insurance & Prop Bets idea as now fully ✅ (was 🟡 evidence-only before item 18 shipped) and the Prediction Market Viewer idea honestly stays 🟡 partial — pool-implied-probability-vs-TxLINE-reference (item 20) is still open.
- **Found and fixed a stale claim while in there:** the README's Proof Integrity section still said "Only goals back a tradeable market today... TxLINE prices no corners/cards markets" — true when written, false since item 18 shipped. Corrected, and added a new **"Parametric prop markets — unpriced pari-mutuel"** evidence subsection with both markets' real PDAs and `create_market` tx links (same signatures as this doc's item-18 log above), which the track-sheet table and the Proof Integrity write-up both now link to.
- Typecheck not required (docs-only pass); no code changed in this entry.

### Progress log — 2026-07-18 (item 14, Protocol Inspector)

- **Item 14 shipped**: `/inspector` at `apps/web/app/inspector/page.tsx`, linked from the header nav. Read-only mini block-explorer scoped to Goalana — no protocol or DB schema changes, no new API endpoints.
  - **Protocol overview**: Goalana Program ID, TxLINE Oracle Program ID, Config PDA (+ `authority`/`marketAuthority`/`settlementAuthority` once fetched), and today's `daily_scores_roots` PDA with a Published/Not-Yet-Published freshness badge. That account has no typed layout anywhere in this repo (TxLINE owns it privately — confirmed via `txline_cpi.rs`/`txoracle_mock`), so "freshness" is necessarily an existence check, not a real last-updated timestamp — documented on the card itself rather than glossed over.
  - **Markets**: every `Market` account via `program.account.market.all()` (on-chain ground truth, not just what's mirrored into Postgres), with a new human-readable predicate formatter (built from `TXLINE_STAT_LABELS` + `op`/`comparison`/`threshold` — no such formatter existed before this), status badge, YES/NO pool balances, and an expandable row with Market PDA, live Vault PDA balance, origin/creator, predicate hash, outcome, and Create/Lock/Settle tx links (joined from `GET /api/markets` for the labels/signatures, since tx history isn't itself an on-chain field).
  - Reused rather than rebuilt: `useGoalanaProgram`, `getConfigPda`/`getVaultPda`/`getDailyScoresRootsPda`, `explorerTxUrl`/`explorerAddressUrl`, `decodeStatus`, `formatDate`/`formatTimeWithZone`, and the existing shadcn `Table`/`Badge`/`Card` primitives — page matches the site's existing dense mono/uppercase data-panel style rather than introducing a new one.
  - **Built out of v3-todo.md's stated order** — v3 §6 deprioritized this item given the 07-19 23:59 UTC deadline and unfinished P0 submission blockers; shipped anyway on explicit instruction. v3-todo.md §6 updated to reflect it's no longer in the "not doing" set.
  - `bun run typecheck` and `bun run lint` both pass clean on the new file (one pre-existing-pattern `@typescript-eslint/no-explicit-any` warning, same as `app/page.tsx:88`). Verified via the running dev server that `/inspector` compiles and serves 200 with no server-side render errors. **Not verified**: an actual browser driving the client-side on-chain fetch end-to-end — no headless-browser tool was available in the sandbox and `npm install playwright` failed with no network access. Recommend opening `http://localhost:3000/inspector` manually once before relying on it in the demo.

### Progress log — 2026-07-18 (item 20, plus v3 P2-2/P2-3 code portions)

User asked for 3 concrete code tasks (not docs) picked from the open items in v2/v3-todo — chose all three below.

- **Item 20 shipped: pool-implied probability vs TxLINE reference on market cards.** `apps/web/app/fixtures/[fixtureId]/page.tsx`: new `PoolVsReference` component renders "POOL IMPLIES X% YES · TXLINE Y%" beneath the existing pool-balance line on every priced market card (`totalYes`/`totalNo` were already read on-chain; `currentYesPct`/`initialYesPct` were already in the `/api/fixtures/:id` response — no new data fetch). Adds a ▲/▼ delta badge only once the two diverge by ≥3 percentage points (`IMPLIED_PROBABILITY_DIVERGENCE_THRESHOLD`), so normal noise doesn't flash a badge on every card. Skipped for parametric prop markets (`isUnpriced`) — the pool already _is_ the price there, no second number to compare against.
- **v3 P2-2 (code only): "permissionless settlement" line on the receipt.** `apps/web/components/fixtures/settlement-proof-receipt.tsx`: added a line under the header ("Permissionless. `settle_market` takes no authority signer — anyone holding this same TxLINE proof could have submitted it. Our keeper is a convenience, not an authority.") shown only in `mode="settled"`, not `mode="preview"` (a preview proof hasn't actually settled anything, so the permissionless claim doesn't apply yet). The matching README paragraph from P2-2 was **not** done here — user asked for code, not docs.
- **v3 P2-3 (code only): judging-window banner.** New `apps/web/components/judging-window-banner.tsx`, wired into `app/layout.tsx` right below `<Header />`. Polls `GET /api/health` independently every 30s (mirrors `txline-health-indicator.tsx`'s pattern rather than sharing its internal state, to keep the two components decoupled); renders nothing when `txline.connected`, renders a slim amber banner explaining the free-tier access window otherwise (and on outright backend-unreachable, since that's the same "looks broken, isn't" case). The matching README note from P2-3 was **not** done here, same reasoning as above.
- All three: `bun run typecheck` clean; `bun run lint` clean on every touched/new file (only pre-existing `any` warnings elsewhere in the large fixture-page file, none from the new code). Verified via the already-running dev server: `/` (banner) and `/fixtures/:id` (market card) both recompiled and served 200 with no server-side error markers. Same caveat as the Inspector entry above — no headless-browser tool available in this sandbox, so the client-side render (banner showing/hiding on real health data, delta badge appearing on a genuinely diverging market) wasn't watched in an actual browser.
- None of these touch the Anchor program or the DB schema — all three are additive frontend-only changes reading data that was already being fetched.

### Progress log — 2026-07-18 (lifecycle animations, SSE backoff cap, B3 close_position)

User asked for a second round of 3 concrete code tasks. Picked: lifecycle-timeline animations (this doc's own open item), the SSE reconnect backoff cap ("Remaining risks" above), and v3-todo's B3 `close_position` — the last one flagged and confirmed before starting, since it jumps v3-todo §4's stated "post-settlement, P0–P2 done" sequencing gate.

- **Lifecycle animations shipped.** `market-lifecycle-timeline.tsx`: each stage row now keys on `${stage.key}-${stage.status}`, so it only remounts (replaying `tw-animate-css`'s `animate-in fade-in slide-in-from-left-1`) when a stage actually transitions, not on every ~10s poll tick. Added `transition-colors duration-500` to the dot, rail connector, label, and status tag so a status flip animates instead of snapping. No new dependency — `tw-animate-css` was already installed and imported globally, just not yet used outside Radix `data-state`-driven components.
- **SSE reconnect backoff cap shipped.** `apps/api/src/workers/odds.worker.ts` and `scorer.worker.ts` (identical pattern in both, confirmed by direct read): replaced the fixed `RECONNECT_DELAY_MS = 5_000` with `RECONNECT_BASE_DELAY_MS = 5_000` / `RECONNECT_MAX_DELAY_MS = 60_000`, doubling on each failed reconnect attempt and resetting to base the moment a connection succeeds (so a brief blip doesn't leave the worker stuck on a slow cadence after TxLINE recovers). Matches exactly what "Remaining risks" above asked for.
- **B3 shipped, not deployed: new `close_position` instruction.** `goalana_program/programs/goalana_program/src/instructions/close_position.rs` — new `ClosePosition` accounts struct (`constraint = position.claimed`, `constraint = position.user == user.key()`, `close = user`), derives its own seeds off `position.market` so it doesn't need the `Market` account at all. New `PositionNotClaimed` error in `error.rs`. Wired into `instructions.rs` and `lib.rs` (`close_position` entrypoint) exactly like every other instruction. Purely additive — doesn't touch `lock_market`/`cancel_market`/any currently-live instruction logic, and doesn't touch any of v3-todo §0's explicitly frozen files (`settle_market.rs`, `txline_cpi.rs`, `txoracle.ts`, `settlement.service.ts`).
- **Verification, B3 especially:** ran the full local suite, not just `anchor build` — `anchor test` **26/26 passing**, confirming nothing regressed. Also confirmed via the freshly generated `target/idl/goalana_program.json` that `close_position` appears with the exact expected PDA seed structure (`position.market` + `user`). Not deployed to Devnet — code + build/test verification only, same posture as A1.
- `bun run typecheck` clean (web + api); `bun run lint` clean on every touched web file (api's lint script is broken in this environment — `eslint: command not found` — confirmed via `git stash` that this predates all of today's changes, not something introduced here). Dev server recompiled `/` and `/fixtures/:id` cleanly after the animation change; only runtime noise was pre-existing public-devnet-RPC 429 rate-limiting on `useMarketAccount`, unrelated to this pass.
- None of these three touch the DB schema. B3 is the one Anchor program change in this entry — everything else is frontend/backend TypeScript.

### Progress log — 2026-07-18 (SDK IDL sync bug)

Prompted by a routine "is the SDK synced with the latest Anchor build?" check after the B3 `close_position` addition above — the answer was no, and the mechanism was worse than a stale copy.

- **Bug found: `sync-idl.ts` was corrupting fixed-address accounts in the generated SDK.** Its `addressRegex` rewrote _every_ `"address": "..."` occurrence when copying `goalana_program/target/idl/goalana_program.json` → `packages/goalana-sdk/src/idl/goalana_program.json` (and the matching `target/types` → `src/types` copy), not just the program's own top-level address. That silently stamped the goalana program's own address over `system_program`'s (should be `11111111111111111111111111111111`) in three places and `txoracle_program`'s (should be `6pW64gN1s2uqjHkn1unFeEjAwJkPGHoppGvS715wyP2J`) in one — confirmed via a direct diff against `target/idl/goalana_program.json`, 4 accounts wrong.
- **Fix:** `sync-idl.ts` now only normalizes the top-level program `address` field — for the JSON copy via `JSON.parse` → mutate `.address` → `JSON.stringify` (so nested account addresses can't be touched, by construction, not by regex luck); for the TS type declaration (not valid JSON, so still text-based) via a non-global regex that only replaces the first occurrence, since the top-level field is always first in Anchor's generated type. Re-ran the sync; diffed the result against `target/idl`/`target/types` — identical apart from a trailing-newline formatting nit.
- **Blast radius was mostly contained but real.** Every caller that explicitly overrides `systemProgram`/`txoracleProgram` in its own `.accountsPartial()` call (`placeBetOnChain`, `claimWinningsOnChain`, `settleMarketOnChain` in `goalana.service.ts`, and apps/web's bet flow in `app/fixtures/[fixtureId]/page.tsx`) was shielded, since the explicit value wins over whatever the corrupted IDL would have auto-resolved. But `initializeGoalanaConfig()` calls `.initializeConfig()` with no accounts at all, relying entirely on Anchor's client-side auto-resolution for `system_program` — that would have silently built a transaction with the wrong system-program account on any fresh environment where config isn't yet initialized. Dormant only because config has already been initialized in every environment this has run in so far.
- **Second bug, surfaced by the fix rather than caused by it:** `createMarketForFixture` used the strict `.accounts({...})` instead of `.accountsPartial({...})`. Once `system_program` carries a real (correct) fixed address, Anchor's generated types exclude it from the strict `.accounts()` call's allowed properties entirely — same class of issue as the seed-derived `market` account hit in the Anchor test suite (worked around there with `as any`, since those are raw IDL-typed test calls). Switched `createMarketForFixture` to `.accountsPartial()` instead, matching the convention every other call in `goalana.service.ts` already followed.
- **Verified:** `turbo run typecheck` — all 6 packages clean (`goalana-sdk`, `api`, `web`, `db`, `ui`, `txline`). `apps/api`'s `market.service.spec.ts` — 8/8 passing. Anchor's own `anchor test` — 29/29 passing (26 pre-existing + 3 new `close_position` tests added alongside this pass; unaffected by the SDK fix itself, but confirms the on-chain side stayed untouched).
- Files touched: `packages/goalana-sdk/scripts/sync-idl.ts` (root-cause fix), `packages/goalana-sdk/src/idl/goalana_program.json` + `src/types/goalana_program.ts` (regenerated correctly), `apps/api/src/services/goalana.service.ts` (`accountsPartial` fix).

### Progress log — 2026-07-18 (shareable links, position/market detail views, competition-discovery revert)

Three more commits landed (`64e6664`, `54b88ab`, `869f520`), reviewed here since neither todo doc tracked them yet.

- **Shareable betting links + OpenGraph + detail views (`64e6664`, `54b88ab`).** New `GET /api/markets/:marketPda` (single off-chain market read, joined with the fixture's current TxLINE reference odds so it returns the same numbers whether reached via the fixture page or a shared link). New pages: `/market/[marketId]`, `/share/bet/[shareId]`, `/share/fixture/[fixtureId]`, `/share/market/[marketId]`. New `apps/web/lib/site.ts` (`getSiteUrl()`, backed by `NEXT_PUBLIC_SITE_URL`) for building absolute share URLs / `metadataBase`. New components: `fixture-detail-view.tsx`, `market-detail-view.tsx`, `market-activity-panel.tsx`, `bet-share-view.tsx`, `share-actions.tsx`, `verification-badge.tsx`; new hooks `use-market-meta.ts`, `use-market-positions.ts`, `use-position-by-pda.ts`, `lib/position-status.ts`, `lib/server-api.ts`.
  - **Note the tension with this doc's own §1E teardown**, which named "share-on-X" as something explicitly **not** to copy from WorldCup PredMarket — generic consumer-app surface, not settlement differentiation. This shipped anyway on (assumed) explicit request; not undone since it's built and typechecked, but it's the kind of feature this doc's original judge-lens reasoning would have deprioritized against the still-open v3-todo P0 items (video, Vercel deploy, repo visibility — see v3-todo.md's 2026-07-18 entry). Worth a conscious call on whether it earns a slot in the 5-minute demo script or gets left as a nice-to-have a judge can stumble onto in the repo.
  - `apps/api/src/index.ts`'s CORS origin now reads `NEXT_PUBLIC_SITE_URL` instead of the previously-unused `FRONTEND_URL` — matches the frontend's own new site-URL convention, but means the API's CORS allowlist is now driven by a `NEXT_PUBLIC_*`-prefixed var; double check this is set correctly in the deployed API's env before P0-2's Vercel deploy, since a mismatch here would silently break wallet/bet requests from the deployed frontend origin.
- **Multi-competition Devnet hedge reverted (`869f520`).** Deleted `apps/api/src/config/competition.ts` (`getActiveCompetitionId()`), the two diagnostic scripts (`verify-competition-discovery.ts`, `check-live-state.ts`), and the README "Validation mode" section — all added in the 2026-07-17 "Multi-competition Devnet validation infrastructure" entry above. Reason (not stated in the commit, inferred from the diff): the hedge was never exercised (Friendlies never got priced odds — see that entry's own "Honest gap"), and World Cup was always going to be the demoed competition, so the extra discovery layer was pure unused surface the day before submission. Back to hardcoded `WORLD_CUP_COMPETITION_ID` constants in `fixtures.cron.ts`/`market.service.ts`/`packages/txline/src/constants.ts`. No functional loss; one less thing to explain on camera or in the README.
- Typecheck not re-verified as part of this review pass (no code changed here — audit only).
