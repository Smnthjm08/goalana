  ...
  function fetchFixtures() {
    ...
    axiosInstance
      ...
      .then((res) => {
        if (res.data?.data) {
          const sorted = [...res.data.data].sort(
            (a: FixtureWithMarkets, b: FixtureWithMarkets) => {
              const aIsLive =
                a.liveStatusId != null &&
                IN_PROGRESS_STATUS_IDS.has(a.liveStatusId)
              const bIsLive =
                b.liveStatusId != null &&
                IN_PROGRESS_STATUS_IDS.has(b.liveStatusId)
      ...
      .catch((err) => {
        console.error(err)
        setError(
          "Could not load fixtures. The feed may be temporarily unavailable."
        )
      })
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-8">
        ...
        <div className="flex flex-col gap-1 border-b border-border pb-6">
          ...
          <p className="font-mono text-[11px] leading-relaxed text-muted-foreground">
            All tracked matches with their on-chain prediction markets. Click a
            fixture to place a bet.
          </p>
        </div>
        ...
        {!loading &&
          ...
          (() => {
            ...
            function renderFixture(
              ...
              const marketCount =
                fixture._count?.markets ?? fixture.markets?.length ?? 0
              ...
              return (
                <div key={fixture.fixtureId} className="flex flex-col gap-3">
                  ...
                  >
                    <div
                      className={`flex items-center justify-between gap-4 rounded-sm border bg-card px-4 py-3 transition-colors ${dimmed ? "border-border/40 opacity-60 hover:border-border hover:opacity-80" : "border-border hover:border-primary/50 hover:bg-card/80"}`}
                    >
                      {/* Teams */}
                  ...
                  {fixture.markets && fixture.markets.length > 0 && (
                    <div className="grid grid-cols-1 gap-2 pl-4 sm:grid-cols-2 lg:grid-cols-3">
                      {fixture.markets.map((market) => (
                        ...
                        >
                          <span className="min-w-0 truncate font-mono text-[10px] leading-snug text-muted-foreground transition-colors group-hover:text-foreground">
                            {market.question}
                          </span>
                          <MarketStatusBadge
                            status={market.status as OnChainMarketStatus}
                          />
                        </Link>
                      ))}
                    </div>
