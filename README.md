...
1. **Create** — the house opens on-chain markets for a real TxLINE World Cup fixture (`competitionId 72`). Creation is **house-only by design** (see [non-goals](docs/TECHNICAL_DOCUMENTATION.md#non-goals-this-hackathon)).
...
Full detail: [`docs/TECHNICAL_DOCUMENTATION.md`](docs/TECHNICAL_DOCUMENTATION.md#part-2--txline-integration).
...
Backend needs a Postgres URL and TxLINE credentials — see [`.env.example`](./.env.example) and [`docs/SETUP.md`](docs/SETUP.md#part-a--vm-deployment-manual-deployment-of-appsapi). World Cup is competition `72`.
