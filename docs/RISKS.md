...
half of that honesty: what _isn't_ trustless, what's operationally fragile, and what we measured
...
("Total corners > 9.5", "Total cards > 3.5") on France v England.
...
can be demonstrated. The localnet suite is still valuable — it exercises Goalana's _own_ guards
...
- Neither power can move a single lamport without a user's own signed claim — the trust surface
  is "the house can change _when_ a market resolves," never "the house can decide _what_ it
...
(caught and corrected before any real settlement proof was submitted) —
